...
$(document).ready(function(){
    ...
    (function( bootstro, $, undefined ) {
        ...
        var defaults = {
            ...
            // Allows us to have mutliple tours on a single page and invoke each one separately
            set : 0
        };
        var settings;
        ...
        get_popup = function(i)
        {
            ...
            p.set = $el.attr('data-bootstro-set');
            if (typeof p.set == typeof undefined || p.set == false) {
                p.set = 0;
            }

            while (p.set!=settings.set && $el) {
                i++;
                $el = get_element(i);
                p.set = $el.attr('data-bootstro-set');
                if (typeof p.set == typeof undefined || p.set == false) {
        ...
        bootstro._start = function(selector)
        {
            ...
            $(selector).each(function(i){
                var eleset = $(this).attr('data-bootstro-set');
                if (typeof eleset == typeof undefined || eleset == false) {
                    eleset = 0;
                }
                if (eleset==settings.set) {
                    $elements[count] = this;
                    count++;
                }
            });
