...
//            return json_encode(array('error' => 'Sorry, but either you or the code was not recognised')) ;
    ...
    public function actionConfirm($key) {
        ...
        // -------------------------------------------------------------------
        // search for userKey. This is of type TYPE_PASSWORD_RESET
        // -------------------------------------------------------------------
        if (($user = $this->validateKey($key) /* || $key==$testToken */)) {
            ...
            // ---------------------------------------------------------------
            // We now create a new key, TYPE_GAPENDING, which the set password
            // code will then search for
            // ---------------------------------------------------------------
    ...
    private function validateKey($key, $keyType = null) {
        ...
        // -------------------------------------------------------------------
        // Now we have a reference to the key type, if no key type was provided,
        // set the default list of types we're happy to search for.
        // -------------------------------------------------------------------
        if (!$keyType) {
            $keyType = [
                $userKeyModel::TYPE_EMAIL_ACTIVATE,
                $userKeyModel::TYPE_EMAIL_CHANGE,
                $userKeyModel::TYPE_PASSWORD_RESET
            ];
        }
        ...
        // -------------------------------------------------------------------
        $userKey = $userKeyModel::findActiveByKey($key, $keyType);
